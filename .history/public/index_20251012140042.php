...
if ($_SERVER["REQUEST_METHOD"] === "POST" && ($_POST['form_type'] ?? '') === 'equipe') {
    ...
    if ($nomEquipe !== "" && $clubId > 0) {
        $equipe = new \App\Model\Equipe(null, $nomEquipe, '', ''); // ville/adresse du club récupérées automatiquement

        $equipeDb = new \App\Database\EquipeDatabase($pdo);
        $success = $equipeDb->addEquipeWithClubAdverse($equipe, $clubId);

        if ($success) {
            $message = "✅ Équipe ajoutée avec succès !";
            // Rafraîchir la liste
            $equipes = $equipeDb->getAll();
        } else {
            $message = "❌ Erreur lors de l'ajout de l'équipe.";
        }
    } else {
        $message = "❌ Veuillez remplir le nom de l'équipe et choisir un club.";
    }
}
    ...
    <select name="club_id" required>
        <?php foreach ($clubs as $club): ?>
            <option value="<?= (int)$club['id'] ?>">
                <?= htmlspecialchars($club['ville']) ?> - <?= htmlspecialchars($club['adresse']) ?>
            </option>
        <?php endforeach; ?>
    </select>
...
<form method="POST">
    ...
    <select name="club_adv_id" required>
        <?php foreach ($clubs as $club): ?>
            <option value="<?= (int)$club['id'] ?>">
                <?= htmlspecialchars($club['ville']) ?> (ID: <?= (int)$club['id'] ?>)
            </option>
        <?php endforeach; ?>
    </select>
