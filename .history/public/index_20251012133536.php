...
if ($_SERVER["REQUEST_METHOD"] === "POST" && ($_POST['form_type'] ?? '') === 'equipe') {
    ...
    $villeEquipe = trim($_POST["ville_equipe"]);

    if ($nomEquipe !== "" && $villeEquipe !== "") {
        // On crée l'objet Equipe avec nom + ville
        $equipe = new Equipe(null, $nomEquipe, $villeEquipe);
        ...
        $message = "❌ Veuillez remplir le nom et la ville de l'équipe.";
    }
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && ($_POST['form_type'] ?? '') === 'match') {
    ...
    $opposingClubId = (int)$_POST['opposing_club_id'];
    ...
    $matchDb = new App\Database\MatchDatabase($pdo);
...
<form method="POST">
    ...
    <label>Ville :</label>
    <input type="text" name="ville_equipe" required>
...
<form method="POST">
    ...
    <select name="opposing_club_id" required>
        <?php foreach ($opposingClubs as $club): ?>
            <option value="<?= $club->getId() ?>">
                <?= htmlspecialchars($club->getCity()) ?> (ID: <?= $club->getId() ?>)
            </option>
        <?php endforeach; ?>
    </select>
