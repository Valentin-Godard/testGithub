...
class Joueur implements Savable {
    ...
    public function save(PDO $pdo): void {
        if ($this->id === null) {
            ...
            $stmt->execute([$this->nom, $this->prenom, $this->birthdate->format("Y-m-d"), $this->getImage(),]);
            $this->id = $pdo->lastInsertId();
        } else {
            $stmt = $pdo->prepare("UPDATE joueurs SET nom=?, prenom=?, date_naissance=?, photo=?, role=? WHERE id=?");
            $stmt->execute([$this->nom, $this->prenom, $this->birthdate->format("Y-m-d"), $this->getImage(), $this->id]);
        }
    }
